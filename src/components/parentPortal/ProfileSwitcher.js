...
const ProfileSwitcher = ({ parentId, onChildSelect, selectedChildId }) => {
  ...
  useEffect(() => {
    const fetchChildren = async () => {
      if (!parentId || !token) {
        setError("Missing parent ID or authentication token");
        setLoading(false);
        return;
      }
      ...
      try {
        setLoading(true);
        const response = await axios.get(`${BASE_URL}/api/parents/${parentId}`, {
          headers: {
        ...
        if (childList.length === 0) {
          ...
          if (!storedChildId || !validStoredChild) {
  ...
  // Ensure the Select value is valid
  const selectValue = children.some((child) => child._id === studentId) ? studentId : "";
  ...
  if (error || !parentId || !token) {
    return (
      <Typography sx={{ color: "#b91c1c", fontWeight: 500, p: 1 }}>
        {error || "Please login to select a child"}
      </Typography>
    );
  }

  const selectedChild = children.find((c) => c._id === selectValue);
