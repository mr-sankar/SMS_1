...
function TeacherDetails() {
      ...
      if (assignmentsResponse.status === 200) {
        ...
        setTimetable((prevTimetable) =>
          prevTimetable.map((slot) => {
            const [className, section] = slot.class
              .split("-")
              .map((s) => s.trim());
  ...
  return (
    <div className="dashboard-container">
      <h1 className="dashboard-title" style= {{color:"#0000ff"}}>Teacher Dashboard</h1>
        ...
        <>
          ...
          <div className="teacher-header mb-4">
            <Row className="align-items-center">
              ...
              <Col md={10}>
                <h2 className="teacher-name Kd-tn text-white">{teacherDetails.name}</h2>
                <p className="teacher-info Kd-info">
                  <span className="Kd-sp">
                    <strong>ID:</strong> {teacherDetails.teacherId}
                  </span>
                  <span className="Kd-sp">
                  ...
                  |
                  <span className="Kd-sp">
                    <strong>Phone No:</strong> {teacherDetails.phoneno}
                  </span>{" "}
                  |
                  <span className="Kd-sp">
                  ...
                  |
                  <span className="Kd-sp">
                    <strong>Joining Date:</strong> {teacherDetails.joiningDate}
                  </span>{" "}
                  |
                  <span className="Kd-sp">
          ...
          {/* Timetable Section */}
          <Card className="timetable-card mb-4 Kd-tb" style={{background:"transparent"}}>
            <Card.Header className="timetable-header Kd-hea">
              <h5 className="Kd-cls text-white">Timetable</h5>
            </Card.Header>
            <Card.Body>
              {timetable.length > 0 ? (
                    ...
                    {timetable.map((slot, index) => {
                      ...
                      return (
                        ...
                        >
                            ...
                            {slot.assignment ? (
                              <div>
                                ...
                                <Badge
                                  className="Kd-bad"
                                  bg={
                                    slot.assignment.type === "Assignment"
                                      ? "info"
                                      : "warning"
                                  }
                                >
                          ...
                          <td style={{ padding: "12px" }}>
                            {!isBreak &&
                              !isLunch && ( // Added !isLunch condition
                                <>
                                  {!slot.assignment && (
                                    <Button
                ...
                <Row>
                  {futureWork.map((slot) => {
                    const daysLeft = Math.ceil(
                      (new Date(slot.assignment.dueDate) - new Date()) /
                        (1000 * 60 * 60 * 24)
                    );
                    return (
                      <Col md={4} key={slot.index} className="mb-3">
                        <Card
                          className={`work-card Kd-cs ${
                            daysLeft < 3 ? "urgent" : ""
                          }`}
                          onMouseEnter={(e) => {
                        ...
                        >
                          <Card.Body>
                            ...
                            <Card.Subtitle className="mb-2 text-muted Kd-til1">
                            ...
                            >
                              ...
                              {slot.assignment.syllabus || "N/A"}
                            </p>
                            <p className="due-date Kd-date1">
                              <strong>Due:</strong>{" "}
                              {new Date(
                                slot.assignment.dueDate
                              ).toLocaleDateString()}
                              {daysLeft < 3 && (
                                <Badge bg="danger" className="ms-2 Kd-bg1">
                                  Urgent
                                </Badge>
                              )}
                            </p>
          ...
          >
            <Modal.Header closeButton className="modal-header Kd-modal">
              <Modal.Title>
                {selectedSlot !== null && timetable[selectedSlot]?.assignment
                  ? "Edit"
                  : "Assign"}{" "}
                Work
                {selectedSlot !== null &&
                  ` for ${timetable[selectedSlot]?.time}`}
                {assignmentForm.className &&
                  assignmentForm.section &&
                  ` - ${assignmentForm.className} ${assignmentForm.section}`}
              </Modal.Title>
            </Modal.Header>

            <Modal.Body className="modal-body Kd-bd">
              <Form>
                {[
                  {
                    label: "Title",
            ...
            <Modal.Footer className="modal-footer Kd-foots" style={{}}>
              <Button
                variant="secondary"
              ...
              <Button
                ...
                disabled={
                  loading || !assignmentForm.title || !assignmentForm.dueDate
                }
                className="modal-btn Kd-bs"
              >
